...
async def create_service_order(data: ServiceOrderCreate):
...
async def list_orders_by_tenant(tenant_id: str):
...
async def set_budget(order_id: str, itens: List[BudgetItem]):
    order = await service_orders_collection.find_one({"id": order_id})
    if not order:
        raise HTTPException(status_code=404, detail="Ordem de serviço não encontrada")
